...
use crate::{BitcoinNetwork, LightningSwapResult, LightningSwapError};
    ...
    T: Clone,
{
    ...
    Err(LightningSwapError::SwapTimeout(format!(
        "Condition not met after {} attempts", max_attempts
    )))
}
...
pub fn parse_btc_amount(amount_str: &str) -> LightningSwapResult<u64> {
    ...
    if amount_str.ends_with(" btc") {
        ...
        let btc: f64 = btc_part.parse()
            .map_err(|_| LightningSwapError::InvalidSwapResponse(
                format!("Invalid BTC amount: {}", amount_str)
            ))?;
        ...
        sat_part.parse()
            .map_err(|_| LightningSwapError::InvalidSwapResponse(
                format!("Invalid satoshi amount: {}", amount_str)
            ))
    } else {
...
mod tests {
    ...
    fn test_get_bitcoin_network() {
        assert_eq!(get_bitcoin_network(BitcoinNetwork::Mainnet), bitcoin::Network::Bitcoin);
        assert_eq!(get_bitcoin_network(BitcoinNetwork::Testnet), bitcoin::Network::Testnet);
        assert_eq!(get_bitcoin_network(BitcoinNetwork::Regtest), bitcoin::Network::Regtest);
        assert_eq!(get_bitcoin_network(BitcoinNetwork::Signet), bitcoin::Network::Signet);
    }
}
